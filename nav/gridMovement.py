...
class GridMovement:
	...
	def __init__(self, grid, queue):
		...
		self.grid = grid
		self.queue = queue
		self.current = (0,0) #Hardcoded should be (4,3)
		self.goal = (0,5) #Hardcoded for now. Should be generated by find_goal()
		self.facing = 0
		self.path = []
		self.movement = 
			{
	...
	def face_goal():
		...
		if( degrees > 0):
			self.queue.put(['turn', (rotr, degrees)])
		elif(degrees < 0):
			self.queue.put(['turn', (rotl, degrees*-1)])
	...
	def translate_dir(self, mov):
		...
		if(self.facing == 0):
			result = mov
		elif(self.facing == 180 || self.facing == -180):
